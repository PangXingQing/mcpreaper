...
如果浏览器不支持播放器，可直接下载音频：[testreaper.wav](ReaperProject/testreaper.wav)
...
6. 脚本会自动创建 `enable_distant_api.txt`，并将 Web Interface 端口固定为 `2307`
...
- 在 Reaper 中执行 `scripts/enable_reapy_server.lua`
- Web Interface 固定监听 `2307`
- MCP 使用本机 IPv4 + 端口 `2307` 连接

连接配置由项目内 `utils/reaper_client.py` 统一管理，不再依赖旧的 `localhost/127.0.0.1/多端口` 回退。
...
然后在 Reaper Action List 中加载并运行 `scripts/enable_reapy_server.lua`。
...
```powershell
# 使用项目内检查脚本（自动选择本机有效 IPv4）
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\check_web_2307.ps1
```
...
A: 常见原因及解决方案：
1. 先确认已在 Reaper 执行 `scripts/enable_reapy_server.lua`。
2. 运行检查脚本：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\check_web_2307.ps1`
3. 端口不通时，检查 Web Interface 是否监听在 `2307`。
4. 尝试运行：`MCP: Restart Server (mcpreaper)`
...
- 本仓库聚焦 Reaper 的 MCP 工具能力，覆盖音轨、媒体项、FX、MIDI、渲染、影视声音制作等完整流程。
- 连接策略已统一为单一路径：Reaper Web Interface 端口 `2307`。
- `ReaperProject/` 当前已替换为测试工程 `testreaper`，用于验证编排与渲染链路。
